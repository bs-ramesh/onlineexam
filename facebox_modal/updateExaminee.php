...
<?php 
  ...
  $selExmne = $conn->query("SELECT * FROM examinee_tbl WHERE usn='$id' ")->fetch(PDO::FETCH_ASSOC);
...
<form method="post" id="updateExamineeFrm">
     ...
     <div class="form-group">
        ...
        <?php 
            $exmneCourse = $selExmne['branch_id'];
         ...
         <select class="form-control" name="exCourse">
           <option value="<?php echo $exmneCourse; ?>"><?php echo $selCourse['branch_name']; ?></option>
           <?php 
             $selCourse = $conn->query("SELECT * FROM branch_tbl WHERE branch_id!='$exmneCourse' ");
     ...
     <div class="form-group">
        <legend>usn</legend>
        <input type="" name="exEmail" class="form-control" required="" value="<?php echo $selExmne['usn']; ?>" >
     </div>
