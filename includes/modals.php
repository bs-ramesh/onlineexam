...
<div class="modal fade myModal" id="updateCourse-<?php echo $selCourseRow['branch_id']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog " role="document">
     <form class="refreshFrm" id="addCourseFrm" method="post" >
       <div class="modal-content myModal-content" >
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">Update ( <?php echo $selCourseRow['branch_name']; ?> )</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <div class="col-md-12">
            <div class="form-group">
              <label>Course</label>
              <input type="" name="course_name" id="course_name" class="form-control" value="<?php echo $selCourseRow['branch_name']; ?>">
            </div>
          </div>
        </div>
        <div class="modal-footer">
     ...
     <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="col-md-12">
              ...
              <?php 
                ...
                if($selCourse->rowCount() > 0)
                {
                  while ($selCourseRow = $selCourse->fetch(PDO::FETCH_ASSOC)) { ?>
                     <option value="<?php echo $selCourseRow['branch_id']; ?>"><?php echo $selCourseRow['branch_name']; ?></option>
                  <?php }
                }
                else
                { ?>
                  <option value="0">No Depratment Found</option>
                <?php }
               ?>
            </select>
          </div>
          ...
          <div class="form-group">
            <label>Course</label>
            <input type="" name="examTitle" class="form-control" placeholder="Course Name" required="">
          </div>

          <div class="form-group">
            <label>Event Title</label>
            <textarea name="examDesc" class="form-control" rows="4" placeholder="Input Event Name" required=""></textarea>
          </div>
        ...
        <div class="col-md-12">
          ...
          <div class="form-group">
            ...
            <select class="form-control" name="course" id="course">
              ...
              <?php 
                $selCourse = $conn->query("SELECT * FROM branch_tbl ORDER BY branch_id asc");
                while ($selCourseRow = $selCourse->fetch(PDO::FETCH_ASSOC)) { ?>
                  <option value="<?php echo $selCourseRow['branch_id']; ?>"><?php echo $selCourseRow['branch_name']; ?></option>
                <?php }
               ?>
          ...
          <div class="form-group">
            <label>USN</label>
            <input type="text" name="email" id="email" class="form-control" placeholder="Enter USN" autocomplete="off" required="">
          </div>
          <div class="form-group">
...
<div class="modal fade" id="modalForAddQuestion" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
   <form class="refreshFrm" id="addQuestionFrm" method="post">
     <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Add Question for <br><?php echo $selExamRow['ex_subject']; ?></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
